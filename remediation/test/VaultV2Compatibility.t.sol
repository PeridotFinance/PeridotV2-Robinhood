...
import { PairConfig } from "baseline/src/libraries/VaultTypes.sol";
...
import { MockOracleGuard } from "baseline/test/mocks/MockOracleGuard.sol";
import { MockLossReserve } from "baseline/test/mocks/MockLossReserve.sol";
import { MockLiquidityAdapter } from "baseline/test/mocks/MockLiquidityAdapter.sol";

contract RobinhoodBoostedVaultV2CompatibilityTest is Test {
    ...
    function testEmergencyDecreaseRecognizesLossAndRetainsGuardForNativeDeficit() external {
        _depositPair(10e18, 1_000e6);
        vm.prank(keeper);
        vault.rebalance(PAIR_ID, block.timestamp + 60);
        ...
        // USDG remains natively under-backed despite the full LP exit. The stock
        // side must not escape ongoing composition exposure through the idle path.
        oracle.setShouldRevert(true);
        vm.prank(stockAccount);
        vm.expectRevert(RobinhoodBoostedVault.EmergencyMode.selector);
        vault.withdrawForSide(PAIR_ID, address(stock), 8e18, receiver, block.timestamp + 60);
