...
Vault positions use standard Uniswap v4 PoolManager/PositionManager through an adapter, with a full-range zero-hook position. Only configured pToken side accounts may deposit/withdraw. Idle liquidity remains exposed to shared strategy loss while LP liquidity is open OR native claims depend on the other token’s surplus. The archived V1 bypassed guarded loss recognition once LP liquidity reached zero; a regression reproduces that defect. The now-installed V2 restricts oracle-free withdrawal to zero LP liquidity with both native claims fully backed. Installation and unchanged accounting were independently verified at block 73,403,815; see [vault correction](VAULT_UPGRADE.md).
